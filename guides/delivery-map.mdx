...
- **History:** Use Show All to see how approaches have matured across iterations. A good approach has a few jobs in an early iteration (validation), then more in a later one (polish). An approach with jobs in every iteration might be too broad — consider splitting it.
